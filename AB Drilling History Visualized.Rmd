...
intervals <- gif_duration * frames_per_second

```


Investigate using list columns and nest() here. Otherwise just use map on the following:

```{r}
wells_short <- wells %>% filter(lubridate::year(`FINAL-DRILL-DATE`) > 2013)
date_edges <- seq(min(wells_short$`FINAL-DRILL-DATE`, na.rm = T), max(wells_short$`FINAL-DRILL-DATE`, na.rm = T), length.out = intervals + 1)
date_starts <- date_edges[1:36]
date_ends <- date_edges[2:37]
