...
well_map <- ggmap(ab) + geom_point(data = wells_May_20_1990, aes(x = mean_longitude, y = mean_latitude, color = "red")) +
  theme(legend.position="none") + 
  ggtitle("Wells Drilled in Alberta Over Time") + 
  geom_text(aes(label = label), data = date_label, vjust = "top", hjust = "left") + 
  labs(
    x = "Longitude",
    y = "Latitude"
  )
...
lay <- rbind(c(1,1,1,2,2),
             c(1,1,1,2,2))
grid.arrange(well_map, well_chart + date_range_geom, layout_matrix = lay)
```
...
```{r}
# The basic well map and well chart will not vary beteen frames and should be create once outside the loop
