...
A simple loop can create the image sequence for the animation. The loop has been wrapped in a function to make the creation of multiple different animations straight forward:
...
animate_well_points <- function(well_data, duration, frames_per_second = 12, fade_duration = 0.5) {
  ...
  for (frame in 0:frame_count) {
    ...
    ggsave(file = str_c("image_sequence/well_points", str_pad(frame, width = filename_padding, side ="left", pad ="0"),".png"), plot = frame_grob)
  }
}
```
...
system(str_c("ffmpeg -y -framerate ", fps, " -i 'image_sequence/well_points%02d.png' -s:v 1280x720 -c:v libx264 -profile:v high -crf 20 -pix_fmt yuv420p wells_animation.mp4"))
```
