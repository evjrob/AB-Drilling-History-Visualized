...
#ggmap(ab) + geom_point(data = wells %>% filter(`FINAL-DRILL-DATE`), aes(x = mean_longitude, y = mean_latitude, color = "red"))
```
...
for (frame in 0:frame_count) {
  ...
  png(filename=paste("image_sequence/", frame,".png"))
  plot(ggmap(ab) + geom_point(data = frame_wells, aes(x = mean_longitude, y = mean_latitude, color = "red"), alpha = frame_wells$ALPHA) + theme(legend.position="none")) 
  dev.off()
}
```
