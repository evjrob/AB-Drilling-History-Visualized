...
```{r}
wells_short <- wells %>% filter(lubridate::year(`FINAL-DRILL-DATE`) >= 2011)
```


A simple loop can create the image sequence for the animation. The loop has been wrapped in a function to make the creation of multiple different animations possible:
...
The function can then be called on the wells_short data set and an animation of all drilling between 2011 and the present is created:
```{r results='hide', message=FALSE, warning=FALSE}
animation_dur <- 6.4
fps <- 12
fade_dur <- 1/4
...
And the resulting video file can be seen below:
